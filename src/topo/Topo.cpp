  ...
  if (cfg.outputStats) {
    if (cfg.aggregateStats) {
      ...
      if (props.count("statistics")) {
        ...
        if (stats.count("num_nds_in")) numNdsBef = stats.at("num_nds_in").get<size_t>();
        if (stats.count("num_edgs_in")) numEdgsBef = stats.at("num_edgs_in").get<size_t>();
        if (stats.count("len_before")) lenBef = stats.at("len_before").get<double>();
        if (stats.count("iters")) iters = stats.at("iters").get<size_t>();
        if (stats.count("time_const")) constrT = stats.at("time_const").get<double>();
        if (stats.count("time_restr_inf")) restrT = stats.at("time_restr_inf").get<double>();
        if (stats.count("time_station_insert")) stationT = stats.at("time_station_insert").get<double>();
        if (stats.count("max_merged_edgs")) maxMergedEdgs = stats.at("max_merged_edgs").get<size_t>();
        if (stats.count("tot_merged_edgs")) totMergedEdgs = stats.at("tot_merged_edgs").get<size_t>();
        if (stats.count("tot_support_graph_edgs"))
          totSupportGraphEdgs = stats.at("tot_support_graph_edgs").get<size_t>();
      }
    } else {
    ...
    util::json::Dict jsonStats = {
        {"statistics",
         util::json::Dict{
             ...
             {"avg_merged_edgs", (static_cast<double>(totMergedEdgs) / static_cast<double>(totSupportGraphEdgs))},
